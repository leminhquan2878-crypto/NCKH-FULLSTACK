...
async function main() {
    ...
    const hash = await bcryptjs_1.default.hash('123456', 12);
    // ─── Users (mapping from mockData) ─────────────────────────────────────────
    console.log('  Creating users...');
    const [staff, owner, councilUser, accounting, archive, reports, admin, chairman, reviewer, secretary, member] = await Promise.all([
    ...
    await Promise.all(fields.map((f, i) => prisma.category.create({ data: { type: 'field', value: f, label: f, sortOrder: i } })));
    // ─── System Config ───────────────────────────────────────────────────────────
